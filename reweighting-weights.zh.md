...
由大数定律与遍历假设，任意观测量 $A$ 在分布 $\rho$ 下的期望，都可以用从 $\rho$ 中采得的样本 $\{\vec x_i\}$ 的平均来估计：
...
如果我们能直接对目标分布采样，用的就是这个估计式。本文余下部分处理的是采样在*有偏*分布 $p_b$ 下进行的情形：第 4 节的重加权估计正是给每一项乘上重要性权重的样本平均，当偏置 $b\equiv 0$ 时便退化为上式。
