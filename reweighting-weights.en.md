...
observable $A(\vec{x})$ on phase space, the time average along a trajectory
$\vec{x}(t)$ is
...
with $\rho(\vec{x})$ the equilibrium distribution in phase space. The hypothesis
asserts
...
configurations $\{\vec{x}_i\}$ generated by a simulation:
